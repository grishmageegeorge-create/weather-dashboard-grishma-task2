...
st.markdown("<p class='main-subtitle'>Real-time weather · 7-day forecasts · Country insights · Up to 5 cities</p>", unsafe_allow_html=True)
...
if search:
    if not cities_input.strip():
        ...
        for i, city in enumerate(cities):
            ...
            if not coords:
                st.error(f"❌ '{city}' not found! Try a more specific name.")
                continue

            weather   = get_weather(coords["lat"], coords["lon"])
            current   = weather.get("current", {})
            daily     = weather.get("daily", {})
            country   = get_country_info(coords["country"])
            ...
            if daily and "temperature_2m_max" in daily:
                ...
                day_codes    = daily.get("weathercode", [0]*7)
                day_icons    = [get_weather_icon(c)[0] for c in day_codes]
